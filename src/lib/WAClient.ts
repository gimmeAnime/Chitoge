...
export default class WAClient extends Base {
  ...
  getHaigusha = async (jid: string): Promise<IHaigusha> => {
    let user = await this.DB.haigusha.findOne({ jid });
    if (!user)
      user = await new this.DB.haigusha({
        jid,
      }).save();
    return user;
  };

  getPokemons = async (jid: string): Promise<IPokemons> => {
    let user = await this.DB.pokemons.findOne({ jid });
    if (!user)
      user = await new this.DB.pokemons({
        jid,
      }).save();
    return user;
  };
...
export enum toggleableGroupActions {
  ...
  gamblingforlife = "gamblingforlife",
  news = "news",
  normal = "normal",
  tsundere = "tsundere",
  wild = "wild",
