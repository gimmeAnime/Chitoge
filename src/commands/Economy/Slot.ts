...
export default class Command extends BaseCommand {
  ...
  run = async (
    ...
    if (!(await this.client.getGroupData(M.from)).gamblingforlife)
      return void M.reply(
        `You can't do this here, Baka! Use *${this.client.config.prefix}support* to get the casino group link.`
      );
    const user = M.sender.jid;
    const time = 45000;
    const cd = await (await this.client.getUser(user)).lastBet;
