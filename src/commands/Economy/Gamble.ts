...
export default class Command extends BaseCommand {
  ...
  run = async (
    ...
    if (!(await this.client.getGroupData(M.from)).gamblingforlife)
      return void M.reply(
        `You can't gamble here, Baka! Use *${this.client.config.prefix}support* to get the gambling group link.`
      );
    const user = M.sender.jid;
    const time = 25000;
    const cd = await (await this.client.getUser(user)).lastGamble;
    ...
    if (isNaN(amount))
      return void M.reply(
        `🟥 *Invalid format. Usage Example - :gamble 100 left*`
      );
    if (amount < 100)
    ...
    if (!directions.includes(luck))
      return void M.reply(
        `🟥 *The direction should be left or right. Example - :gamble ${amount} left.*`
      );
    if (luck !== direction) {
      await this.client.reduceGold(user, amount);
      await this.client.DB.user.updateOne(
        { jid: user },
        { $set: { lastGamble: Date.now() } }
      );
