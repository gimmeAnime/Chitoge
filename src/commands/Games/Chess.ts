...
export default class Command extends BaseCommand {
  ...
  challenges = new Map<
    string,
    { challenger: string; challengee: string } | undefined
  >();
    ...
    switch (args[0].toLowerCase()) {
      ...
      case "challenge":
        ...
        return void M.reply(
          `@${M.sender.jid.split("@")[0]} has Challenged @${
            challengee.split("@")[0]
          } to a chess match. Use *${
            this.client.config.prefix
          }chess accept* to start the challenge`,
          MessageType.text,
          undefined,
      ...
      case "accept":
        ...
        game.start(
          ...
          async () => {
            const cig = new CIG();
            cig.loadArray(
              this.parseBoard(game.board.getPieces(game.white, game.black))
            );
            let sent = false;
            while (!sent) {
              try {
        ...
        if (args[1] == "castle") {
          ...
          return void g.eventEmitter.emit(
            ...
            () => async () => {
              const cig = new CIG();
              cig.loadArray(
                this.parseBoard(g.board.getPieces(g.white, g.black))
              );
              let sent = false;
              while (!sent) {
                try {
                  await cig
                    .generateBuffer()
                    .then(
                      async (data) => await M.reply(data, MessageType.image)
                    );
                  sent = true;
                } catch (err) {
                  continue;
          ...
          async () => {
            ...
            while (!sent) {
              try {
                await cig
                  .generateBuffer()
                  .then(async (data) => await M.reply(data, MessageType.image));
                sent = true;
              } catch (err) {
                continue;
              }
      ...
      case "ff":
        ...
        if (players.includes(M.sender.jid)) {
          ...
          return void end(
            players.filter((player) => M.sender.jid !== player)[0]
          );
        }
        return void M.reply("You are not participating in any games");
      default:
        return void M.reply(
          `Invalid Usage Format. Use *${this.client.config.prefix}chess* for more info`
        );
    }
  };
}
